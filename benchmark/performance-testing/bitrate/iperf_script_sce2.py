...
csv_file = "iperf3-results-inference-to-mq.csv"

# IP and port of the server to connect to
server_ip = "ziti.rabbitmq"
port = "5201"
client_port = "5203"
time = "20"
...
def run_iperf3():
    ...
    try:
        ...
        if intervals and intervals[0]["streams"][0]["start"] == 0:
            ...
            print(
                f"Excluding first interval with bitrate: {first_packet_bitrate / 1_000_000:.2f} Mbits/sec"
            )
        ...
        if intervals:
            total_sender_bitrate = sum(
                interval["streams"][0]["bits_per_second"] for interval in intervals
            )
            average_sender_bitrate = (
                total_sender_bitrate / len(intervals) / 1_000_000
            )  # Convert to Mbits/sec
        else:
            average_sender_bitrate = 0

        # Calculate the average receiver bitrate of the remaining intervals
        average_receiver_bitrate = (
            output_json["end"]["sum_received"]["bits_per_second"] / 1_000_000
        )  # Convert to Mbits/sec
    ...
    with open(csv_file, mode="a", newline="") as file:
        ...
        if not file_exists:
            writer.writerow(
                [
                    "Timestamp",
                    "Sender Bitrate (Mbits/sec)",
                    "Receiver Bitrate (Mbits/sec)",
                ]
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run iperf3 test multiple times and record results."
    )
    parser.add_argument(
        "-n",
        "--num-runs",
        type=int,
        default=1,
        help="Number of times to run the iperf3 test",
    )

...
ziti edge create identity "preprocessing-$(cat /etc/hostname)" \
  --jwt-output-file /tmp/preprocessing.jwt --role-attributes preprocessing,edge

ziti edge create identity "ensemble-$(cat /etc/hostname)" \
  --jwt-output-file /tmp/ensemble.jwt --role-attributes ensemble,edge,cloud

ziti edge create identity "mobilenetv2-$(cat /etc/hostname)" \
  --jwt-output-file /tmp/mobilenetv2.jwt --role-attributes mobilenetv2,edge

ziti edge create identity "efficientnetb0"-$(cat /etc/hostname) \
  --jwt-output-file /tmp/efficientnetb0.jwt --role-attributes efficientnetb0,edge
...
ziti edge create service "loadbalancer-service" \
  --configs client-intercept-config,loadbalancer-host-config --role-attributes edge
...
#ziti edge create service-edge-router-policy allow.edge5 --edge-router-roles '#edge' --service-roles '@loadbalancer-service'
...
ziti edge create service "preprocessing-service" \
  --configs loadbalancer-intercept-config,preprocessing-host-config --role-attributes edge
...
ziti edge create service "ensemble-service" \
  --configs preprocessing-intercept-config,ensemble-host-config --role-attributes edge,cloud
...
ziti edge create service "mobilenetv2-service" \
  --configs ensemble-intercept-mobilenetv2-config,mobilenetv2-host-config --role-attributes edge
...
ziti edge create service "efficientnetb0-service" \
  --configs ensemble-intercept-efficientnetb0-config,efficientnetb0-host-config --role-attributes edge
...
ziti edge create service "message-queue-service" \
  --configs ensemble-intercept-message-queue-config,message-queue-host-config --role-attributes cloud
...
ziti edge create service-policy "message-queue-dial-policy" Dial \
  --service-roles '@message-queue-service' --identity-roles '#ensemble'
...
ziti edge create service "database-service" \
  --configs message-queue-intercept-database-config,database-host-config --role-attributes cloud
...
ziti edge create service-policy "database-dial-policy" Dial \
  --service-roles '@database-service' --identity-roles '#message-queue'
...
# ziti edge create service-edge-router-policy "public-routers" \
#   --edge-router-roles '#public-routers' --service-roles '#all'
ziti edge create edge-router-policy allow.id.router_edge --edge-router-roles '#edge' --identity-roles '#edge'
ziti edge create service-edge-router-policy allow.svc.router_edge --edge-router-roles '#edge' --service-roles '#edge'

ziti edge create edge-router-policy allow.id.router_cloud --edge-router-roles '#cloud' --identity-roles '#cloud'
ziti edge create service-edge-router-policy allow.svc.router_cloud --edge-router-roles '#cloud' --service-roles '#cloud'

...
# --- Function to create Ziti identity ---
create_ziti_identity() {
  local name="$1"
  local roles="$2"
  log "Creating Ziti identity: $name"
  "$ZITI_CLI" edge create identity "${name}-$(cat /etc/hostname)" \
    --jwt-output-file "/tmp/${name}.jwt" --role-attributes "$roles"
  "$ZITI_CLI" edge enroll --jwt "/tmp/${name}.jwt" --out "/tmp/${name}.json"
}
...
main() {
  ...
  create_ziti_identity "object-detection-client" "object-detection-client,edge"
  
  
  create_ziti_identity "preprocessing" "preprocessing,edge"
  ...
  create_ziti_identity "ensemble" "ensemble,edge,cloud"
  ...
  create_ziti_identity "mobilenetv2" "mobilenetv2,edge"
  ...
  create_ziti_identity "efficientnetb0" "efficientnetb0,edge"
  ...
  create_ziti_identity "loadbalancer" "loadbalancer"
  ...
  create_ziti_identity "message-queue" "message-queue,cloud"
  ...
  create_ziti_identity "database" "database,cloud"

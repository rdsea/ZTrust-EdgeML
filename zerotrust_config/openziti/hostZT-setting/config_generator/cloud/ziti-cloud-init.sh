...
configure_dns() {
  ...
  echo "$PUBLIC_IP ctrl.cloud.hong3nguyen.com" | sudo tee -a /etc/hosts
}
...
create_edge_router() {
  ...
  local jwt_file
  jwt_file="/home/hong3nguyen/$1.jwt"
  ...
  ${ZITI_HOME}/bin/ziti edge create edge-router cloud_router \
    --jwt-output-file $jwt_file \
    --tunneler-enabled

  log "Assigning attribute 'cloud' to edge router"
  ${ZITI_HOME}/bin/ziti edge update edge-router cloud_router -a cloud
...
main() {
  ...
  create_edge_router "cloud_router"

  bootstrap_router "/home/hong3nguyen/cloud_router.jwt"

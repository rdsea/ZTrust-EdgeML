...
bars1 = ax.bar([p - width/2 for p in x], baseline_values, width=width, label='Baseline')
bars2 = ax.bar([p + width/2 for p in x], scenario_values, width=width, label='Scenario 2') 

# Labeling
# ax.set_xlabel('Bitrate Type')
ax.set_ylabel('Bitrate (Mbits/sec)')
ax.set_title('Comparison of Average Sender and Receiver Bitrates for Edge 1 to Edge 2 Layers Traffic')
ax.set_xticks(x)
